...
def test_sparse_vector_strategy_raises_errors():
    strategy = AsyncSparseVectorStrategy("my_model_id")
...
def test_dense_vector_strategy_raises_error():
    with pytest.raises(ValueError):
        # unknown distance
        AsyncDenseVectorStrategy(hybrid=True, text_field=None)

    with pytest.raises(ValueError):
        # unknown distance
        AsyncDenseVectorStrategy(distance="unknown distance").es_mappings_settings(
...
def test_dense_vector_script_score_strategy_raises_error():
    with pytest.raises(ValueError):
        ...
        AsyncDenseVectorScriptScoreStrategy().es_query(
            query=None,
            query_vector=None,
            text_field="text_field",
            vector_field="vector_field",
    ...
    with pytest.raises(ValueError):
        # unknown distance
        AsyncDenseVectorScriptScoreStrategy(distance="unknown distance").es_query(
            query=None,
            query_vector=[1, 2, 3],
            text_field="text_field",
            vector_field="vector_field",
