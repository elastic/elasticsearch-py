...
class TestStreamingBulk(ElasticsearchTestCase):
    def test_actions_remain_unchanged(self):
        ...
        for ok, item in helpers.streaming_bulk(
            self.client, actions, index="test-index", doc_type="answers"
        ):
            self.assertTrue(ok)
    ...
    def test_all_documents_get_inserted(self):
        ...
        for ok, item in helpers.streaming_bulk(
            self.client, docs, index="test-index", doc_type="answers", refresh=True
        ):
            self.assertTrue(ok)

        self.assertEquals(
            100, self.client.count(index="test-index", doc_type="answers")["count"]
        )
        self.assertEquals(
            {"answer": 42},
            self.client.get(index="test-index", doc_type="answers", id=42)["_source"],
        )
    ...
    def test_different_op_types(self):
        ...
        self.assertEquals(
            {"answer": 42}, self.client.get(index="i", doc_type="t", id=42)["_source"]
        )
        self.assertEquals(
            {"f": "v"}, self.client.get(index="i", doc_type="t", id=47)["_source"]
        )
...
class TestBulk(ElasticsearchTestCase):
    def test_bulk_works_with_single_item(self):
        ...
        self.assertEquals(
            1, self.client.count(index="test-index", doc_type="answers")["count"]
        )
        self.assertEquals(
            {"answer": 42},
            self.client.get(index="test-index", doc_type="answers", id=1)["_source"],
        )
...
class TestScan(ElasticsearchTestCase):
    def test_order_can_be_preserved(self):
        ...
        for x in range(100):
            bulk.append(
                {"index": {"_index": "test_index", "_type": "answers", "_id": x}}
            )
    ...
    def test_all_documents_are_read(self):
        ...
        docs = list(
            helpers.scan(self.client, index="test_index", doc_type="answers", size=2)
        )
...
class TestReindex(ElasticsearchTestCase):
    ...
    def test_reindex_passes_kwargs_to_scan_and_bulk(self):
        ...
        self.assertEquals(
            ...
            self.client.get(index="prod_index", doc_type="post", id=42)["_source"],
        )
...
class TestParentChildReindex(ElasticsearchTestCase):
    ...
    def test_children_are_reindexed_correctly(self):
        ...
        q = self.client.get(index="test-index", doc_type="post", id=47, routing=42)
        self.assertEquals(
            {
                "_routing": "42",
                "_id": "47",
