...
class TestStreamingBulk(object):
    def test_actions_remain_unchanged(self, es_client):
        actions = [{"_id": 1}, {"_id": 2}]
        for ok, item in helpers.streaming_bulk(es_client, actions, index="test-index"):
            assert ok
        assert [{"_id": 1}, {"_id": 2}] == actions

    def test_all_documents_get_inserted(self, es_client):
        ...
        assert 100 == (es_client.count(index="test-index"))["count"]
        assert {"answer": 42} == (es_client.get(index="test-index", id=42))["_source"]
    ...
    def test_documents_data_types(self, es_client):
        ...
        es_client.delete_by_query(index="test-index", body={"query": {"match_all": {}}})
    ...
    def test_all_errors_from_chunk_are_raised_on_failure(self, es_client):
    ...
    def test_different_op_types(self, es_client):
        es_client.index(index="i", id=45, body={})
        es_client.index(index="i", id=42, body={})
        docs = [
            {"_index": "i", "_id": 47, "f": "v"},
            {"_op_type": "delete", "_index": "i", "_id": 45},
            {"_op_type": "update", "_index": "i", "_id": 42, "doc": {"answer": 42}},
        ]
        for ok, item in helpers.streaming_bulk(es_client, docs):
            assert ok

        assert not es_client.exists(index="i", id=45)
        assert {"answer": 42} == (es_client.get(index="i", id=42))["_source"]
        assert {"f": "v"} == (es_client.get(index="i", id=47))["_source"]

    def test_transport_error_can_becaught(self, es_client):
        ...
        results = [
            x
            for x in helpers.streaming_bulk(
                failing_client,
                docs,
    ...
    def test_rejected_documents_are_retried(self, es_client):
        ...
        es_client.indices.refresh(index="i")
        res = es_client.search(index="i")
        assert {"value": 3, "relation": "eq"} == res["hits"]["total"]
        assert 4 == failing_client._called

    def test_rejected_documents_are_retried_at_most_max_retries_times(self, es_client):
        failing_client = FailingBulkClient(
            es_client,
            fail_at=(1, 2),
            fail_with=ApiError(
    ...
    def test_transport_error_is_raised_with_max_retries(self, es_client):
        failing_client = FailingBulkClient(
            es_client,
            fail_at=(1, 2, 3, 4),
            fail_with=ApiError(
...
class TestBulk(object):
    def test_bulk_works_with_single_item(self, es_client):
        ...
        assert 1 == success
        assert not failed
        assert 1 == (es_client.count(index="test-index"))["count"]
        assert {"answer": 42} == (es_client.get(index="test-index", id=1))["_source"]
    ...
    def test_stats_only_reports_numbers(self, es_client):
    ...
    def test_errors_are_reported_correctly(self, es_client):
        es_client.indices.create(
            index="i",
            mappings={"properties": {"a": {"type": "integer"}}},
            settings={"number_of_shards": 1, "number_of_replicas": 0},
    ...
    def test_error_is_raised(self, es_client):
        es_client.indices.create(
            index="i",
            mappings={"properties": {"a": {"type": "integer"}}},
            settings={"number_of_shards": 1, "number_of_replicas": 0},
        ...
        with pytest.raises(helpers.BulkIndexError):
            helpers.bulk(es_client, [{"a": 42}, {"a": "c"}], index="i")

    def test_ignore_error_if_raised(self, es_client):
        ...
        helpers.bulk(es_client, [{"a": 42}, {"a": "c"}], index="i", ignore_status=400)
    ...
    def test_errors_are_collected_properly(self, es_client):
        es_client.indices.create(
            index="i",
            mappings={"properties": {"a": {"type": "integer"}}},
            settings={"number_of_shards": 1, "number_of_replicas": 0},
...
def scan_teardown(es_client):
    yield
    es_client.clear_scroll(scroll_id="_all")


class TestScan(object):
    def test_order_can_be_preserved(self, es_client, scan_teardown):
        bulk = []
        for x in range(100):
            bulk.append({"index": {"_index": "test_index", "_id": x}})
            bulk.append({"answer": x, "correct": x == 42})
        es_client.bulk(operations=bulk, refresh=True)
    ...
    def test_all_documents_are_read(self, es_client, scan_teardown):
        bulk = []
        for x in range(100):
            bulk.append({"index": {"_index": "test_index", "_id": x}})
            bulk.append({"answer": x, "correct": x == 42})
        ...
        docs = [x for x in helpers.scan(es_client, index="test_index", size=2)]

        assert 100 == len(docs)
        assert set(map(str, range(100))) == set(d["_id"] for d in docs)
        assert set(range(100)) == set(d["_source"]["answer"] for d in docs)

    def test_scroll_error(self, es_client, scan_teardown):
        bulk = []
        for x in range(4):
    ...
    def test_initial_search_error(self, es_client, scan_teardown):
        with patch.object(es_client, "options", return_value=es_client), patch.object(
            es_client, "clear_scroll", new_callable=SyncMock
        ):
            with patch.object(
                es_client,
                "search",
    ...
    def test_no_scroll_id_fast_route(self, es_client, scan_teardown):
        with patch.object(es_client, "options", return_value=es_client), patch.object(
            es_client, "scroll"
            ...
            data = [x for x in helpers.scan(es_client, index="test_index")]

            assert data == []
            scroll_mock.assert_not_called()
            clear_mock.assert_not_called()

    def test_logger(self, caplog: pytest.LogCaptureFixture, es_client, scan_teardown):
    ...
    def test_clear_scroll(self, es_client, scan_teardown):
        bulk = []
        for x in range(4):
            bulk.append({"index": {"_index": "test_index"}})
            bulk.append({"value": x})
        ...
        with patch.object(es_client, "options", return_value=es_client), patch.object(
            ...
            _ = [x for x in helpers.scan(es_client, index="test_index", size=2)]
    ...
    def test_scan_auth_kwargs_forwarded(self, es_client, scan_teardown, kwargs):
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    data = [
                        x for x in helpers.scan(es_client, index="test_index", **kwargs)
                    ]
    ...
    def test_scan_auth_kwargs_favor_scroll_kwargs_option(
...
def test_scan_from_keyword_is_aliased(es_client, scan_kwargs):
    with patch.object(es_client, "options", return_value=es_client), patch.object(
        es_client,
        "search",
        return_value=MockResponse(
        ...
        [x for x in helpers.scan(es_client, index="test_index", **scan_kwargs)]
        assert search_mock.call_args[1]["from_"] == 1
        assert "from" not in search_mock.call_args[1]


@pytest.fixture(scope="function")
def reindex_setup(es_client):
    bulk = []
    for x in range(100):
        bulk.append({"index": {"_index": "test_index", "_id": x}})
        bulk.append(
...
class TestReindex(object):
    def test_reindex_passes_kwargs_to_scan_and_bulk(self, es_client, reindex_setup):
        helpers.reindex(
            es_client,
            "test_index",
            "prod_index",
        ...
        assert es_client.indices.exists(index="prod_index")
        assert 50 == (es_client.count(index="prod_index", q="type:answers"))["count"]

        assert {"answer": 42, "correct": True, "type": "answers"} == (
            es_client.get(index="prod_index", id=42)
        )["_source"]

    def test_reindex_accepts_a_query(self, es_client, reindex_setup):
        helpers.reindex(
            es_client,
            "test_index",
            "prod_index",
    ...
    def test_all_documents_get_moved(self, es_client, reindex_setup):
        helpers.reindex(es_client, "test_index", "prod_index")
        es_client.indices.refresh()

        assert es_client.indices.exists(index="prod_index")
        assert 50 == (es_client.count(index="prod_index", q="type:questions"))["count"]
        assert 50 == (es_client.count(index="prod_index", q="type:answers"))["count"]
...
@pytest.fixture(scope="function")
def parent_reindex_setup(es_client):
    body = {
        "settings": {"number_of_shards": 1, "number_of_replicas": 0},
        "mappings": {
            "properties": {
    ...
    es_client.indices.create(index="test-index", body=body)
    es_client.indices.create(index="real-index", body=body)

    es_client.index(index="test-index", id=42, body={"question_answer": "question"})
    es_client.index(
        index="test-index",
        id=47,
        routing=42,
...
class TestParentChildReindex:
    def test_children_are_reindexed_correctly(self, es_client, parent_reindex_setup):
        helpers.reindex(es_client, "test-index", "real-index")

        q = es_client.get(index="real-index", id=42)
        assert {
            "_id": "42",
            "_index": "real-index",
            "_primary_term": 1,
        ...
        q = es_client.get(index="test-index", id=47, routing=42)
        assert {
            "_routing": "42",
            "_id": "47",
            "_index": "test-index",
...
@pytest.fixture(scope="function")
def reindex_data_stream_setup(es_client):
    ...
    es_client.indices.create_data_stream(name="py-test-stream")
    es_client.indices.refresh()
    yield
...
class TestAsyncDataStreamReindex(object):
    ...
    def test_reindex_index_datastream(
        ...
        assert es_client.indices.exists(index="py-test-stream")
        assert (
            50 == (es_client.count(index="py-test-stream", q="type:answers"))["count"]
        )
