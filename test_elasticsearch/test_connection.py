...
from elasticsearch.connection import RequestsHttpConnection, \
    Urllib3HttpConnection, THRIFT_AVAILABLE, ThriftConnection, AlfHttpConnection
...
class TestRequestsConnection(TestCase):
    def _get_mock_connection(self, connection_params={}, status_code=200, response_body='{}'):
        con = AlfHttpConnection(**connection_params)
        def _dummy_send(*args, **kwargs):
            dummy_response = Mock()
            dummy_response.headers = {}
            dummy_response.status_code = status_code
    ...
    def test_timeout_set(self):
        con = AlfHttpConnection(timeout=42)
        self.assertEquals(42, con.timeout)
    ...
    def test_oauth(self):
        con = AlfHttpConnection(oauth={
            'token_endpoint': '/token',
            'client_id': 'client_id',
            'client_secret': 'client_secret'
        })
        self.assertEquals('/token', con.session._token_endpoint)
        self.assertEquals('client_id', con.session._client_id)
        self.assertEquals('client_secret', con.session._client_secret)

    def test_repr(self):
        con = self._get_mock_connection({"host": "elasticsearch.com", "port": 443})
        self.assertEquals('<AlfHttpConnection: http://elasticsearch.com:443>', repr(con))
