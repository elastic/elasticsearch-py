...
class SemanticText(Field):
    ...
    :arg inference_id: Inference endpoint that will be used to generate
        ...
        specified, it will default to `.jina-embeddings-v5-text-small` if
        the cluster is authorized to use the Elastic Inference Service,
        otherwise it will default to `.elser-2-elasticsearch`. The
        `.elser-2-elasticsearch` inference endpoint relies on a local ML
        node to run the ELSER model. Defaults to `.jina-
        embeddings-v5-text-small` if omitted.
