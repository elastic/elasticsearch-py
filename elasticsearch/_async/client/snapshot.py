...
class SnapshotClient(NamespacedClient):
    ...
    async def create(
        ...
        *,
        ...
        :param partial: If `true`, allows the snapshot to proceed even if some of the
            target shards are unavailable. In this case, the resulting snapshot will
            not contain snapshots of the unavailable target shards, and will report its
            state as `PARTIAL`. Additionally, if `true`, allows index metadata operations
            such as deletions while the snapshot is in progress. This is almost always
            preferable to failing the snapshot completely when a single shard is unavailable,
            and blocking index metadata operations. If `false`, the entire restore operation
            will fail if one or more indices included in the snapshot do not have all
            primary shards available, and index metadata operations such as deletions
            will be forbidden until the snapshot completes.
    ...
    async def delete(
        ...
        *,
        ...
        :param master_timeout: The period to wait for the master node. If the master
            node is not available before the timeout expires, the request fails and returns
            an error. To indicate that the request should never timeout, set it to `-1`.
        :param wait_for_completion: If `false`, the request returns a response as soon
            as the deletes are scheduled. If `true`, the request returns a response when
            the matching snapshots are all deleted, and the post-deletion cleanup work
            associated with the request has completed. If you make several requests to
            the delete-snapshots API targetting overlapping collections of snapshots
            then some of those requests may perform different parts of the associated
            post-deletion cleanup work, returning their responses at different times.
            For example, if you make two requests to delete the same snapshot then sometimes
            all of the post-deletion cleanup work will be associated with the first request,
            delaying its response, while the second request has no associated post-deletion
            cleanup work and receives its response as soon as the snapshot has been deleted.
        """
        if repository in SKIP_IN_PATH:
            raise ValueError("Empty value passed for parameter 'repository'")
        if snapshot in SKIP_IN_PATH:
