...
class SecurityClient(NamespacedClient):
    ...
    def put_role_mapping(
        ...
        *,
        ...
        :param name: Role-mapping name
        :param enabled: Mappings that have enabled set to false are ignored when role mapping
            is performed.
        :param metadata: Additional metadata that helps define which roles are assigned to
            each user. Within the metadata object, keys beginning with _ are reserved for
            system usage.
        :param refresh: If `true` (the default) then refresh the affected shards to make
            this operation visible to search, if `wait_for` then wait for a refresh to
            make this operation visible to search, if `false` then do nothing with refreshes.
